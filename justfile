...
bootstrap: build-all
    # `install-self` exercises the same stdlib body-injection path
    # production `osty build` walks (`PrepareEntry` always injects
    # bodied stdlib helpers). `OSTY_STAGE0_FALLBACK` is the
    # chicken-and-egg fix from PR #1989 for fresh clones.
