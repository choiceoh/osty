...
bootstrap: build-all
    # Stdlib body lowering is unconditional in `internal/backend` (the
    # former `OSTY_STDLIB_BODY_LOWER` escape hatch was removed). This
    # recipe exercises the same path production `osty build` walks.
    #
    # `OSTY_STAGE0_FALLBACK="${VAR:-1}"` stays as documented above —
    # it's the chicken-and-egg fix from PR #1989.
