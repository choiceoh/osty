...
bootstrap: build-all
    # `install-self` exercises the same stdlib-body injection path as
    # production `osty build`. The CI gate
    # (`fresh-clone-source-bootstrap.yml`) catches regressions that only
    # surface with body injection enabled.
    #
