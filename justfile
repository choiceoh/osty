...
native_llvmgen_exec_tests := "Test(TrySourceUsesEnvBinaryAndDecodesResponse|TryPackageUsesManagedBinaryWhenEnvUnset|TryPackageLibraryForSymbolsForwardsRequiredSymbols|RequestFromMIREncodesPayload)"
...
bootstrap: build-all
    ...
    # sites that previously needed body-lowering disabled to dodge the
    # closure-arg-drop bug introduced by PR #1998's default-on flip.
    # `install-self` now works under the production path, so the
    # bootstrap recipe exercises the same path production `osty build`
    # walks.
    #
    # `OSTY_STAGE0_FALLBACK="${VAR:-1}"` stays as documented above —
    # it's the chicken-and-egg fix from PR #1989 and is independent
    # of stdlib body injection.
