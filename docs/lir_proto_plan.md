...
**Status (2026-06): shipped and gate removed.** The interim
`OSTY_LLVM_LIR_PROTO` env-var scaffold (`internal/llvmgen/lir_proto_gate.go`,
`LIRProtoSelected()`, `ErrLIRProtoNotWired`, and the dispatch warning pins
`TestLLVMDispatchAppendsLIRProtoFallbackWarning` /
`TestLLVMDispatchSkipsLIRProtoWarningWhenGateOff`) was deleted once LIR Proto
became the production subprocess path via `cmd/osty-native-lirproto` and
`tryMIRRequestViaLIRProto` in `cmd/osty-native-llvmgen`. Do not reintroduce
the env gate — routing is now artifact-managed under
`.osty/toolchain/<ver>/osty-native-lirproto`.

Historical deliverables (for archaeology only):

- ~~Add an explicit gate (`OSTY_LLVM_LIR_PROTO=1`).~~
- Route the selected backend entry point through LIR Proto when enabled.
...
- Gate off: output unchanged (pre-wiring era).
- Gate on → wired runner: selected fixtures use LIR Proto and pass.
- Unsupported prototype shapes fall back cleanly.

Current wiring: `internal/nativelirproto` + `internal/toolchain.EnsureNativeLIRProto`
build and invoke the managed `osty-native-lirproto` binary; see
`cmd/osty-native-llvmgen/main.go::tryMIRRequestViaLIRProto`.
