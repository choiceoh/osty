...
> **Retired (PR #2029):** The interim `OSTY_LLVM_LIR_PROTO` env gate,
> `internal/llvmgen/lir_proto_gate.go`, and the `LirLowerConfig.featureGates`
> helpers were removed once LIR Proto became the steady-state subprocess path.
> The historical scaffold below is kept for context only.
...
Design decision: the first Phase-7 slice landed the gate scaffold without
the runner. `internal/llvmgen/lir_proto_gate.go` exposed `LIRProtoEnvVar`
...
sentinel that said "you asked for LIR Proto; the Go-side runner that
would call into `toolchain/lir_proto.osty` does not exist yet; falling
back to the current path").

`internal/backend/llvm.go::generateLLVMIR` read the gate at the very top
of the dispatcher: when set, `ErrLIRProtoNotWired` was appended to the
warnings slice and the dispatcher continued through whichever fallback
path it would have chosen (native-owned fast path or MIR-direct), so
flipping the gate on early stayed safe — production output was unchanged
but the gate selection was visible in build logs and test output. The
native-owned fast path was simultaneously fixed to forward the outer
warnings instead of overwriting them, so the Phase-7 warning was never
silently dropped on the shorter dispatch route.

Pinned by two Go tests: `TestLLVMDispatchAppendsLIRProtoFallbackWarning`
asserted the gate-on path emitted the sentinel; the negative pin
`TestLLVMDispatchSkipsLIRProtoWarningWhenGateOff` asserted the default
behavior was unchanged so a regression that always-on'd the warning would
fail the test instead of silently noisifying every build. Five env-var
unit tests covered the truthy/falsy parsing rules.

The next Phase-7 slice landed the actual MIR -> LIR Proto -> LLVM text
runner — at that point the `ErrLIRProtoNotWired` return was replaced with
a real call into the Osty-owned lowerer plus a structured unsupported-
diagnostic fallback when the prototype declined. PR #2029 removed the
interim gate and its tests once the subprocess path was default.
