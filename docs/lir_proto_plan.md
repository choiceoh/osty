...
> **2026-06-25 update:** The interim `OSTY_LLVM_LIR_PROTO` env gate and
> `internal/llvmgen/lir_proto_gate.go` scaffold were never shipped to
> production as a long-lived switch. LLVM dispatch now routes MIR → LLVM
> IR through the native-owned subprocess (`osty-native-llvmgen` →
> `nativelirproto` → `toolchain/lir_proto.osty`) with no separate host-side
> env gate (`internal/backend/llvm.go::generateLLVMIR`). The historical
> `TestLLVMDispatchAppendsLIRProtoFallbackWarning` pins were removed when
> the gate plumbing was deleted (PR #2029).

Deliverables (historical plan — gate superseded by direct subprocess wiring):

- ~~Add an explicit gate, for example `OSTY_LLVM_LIR_PROTO=1`~~
- Route the selected backend entry point through LIR Proto.
- Keep automatic fallback to the current MIR emitter on structured unsupported
  diagnostics.
- Add focused backend dispatch tests.

Exit criteria (met via subprocess path, not env gate):

- Default path: native-owned LIR Proto subprocess when coverage allows.
- Unsupported prototype shapes fall back cleanly to MIR-direct emitter.

Design decision (historical): the first Phase-7 slice planned a gate scaffold
without the runner. That interim gate was retired once the
`nativelirproto` subprocess became the production integration point.

The next Phase-7 slice landed the actual MIR → LIR Proto → LLVM text
runner — the `ErrLIRProtoNotWired` sentinel path was removed along with
the env gate when subprocess wiring shipped.
