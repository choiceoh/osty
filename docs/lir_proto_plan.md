...
`internal/llvmgen/lir_proto_gate.go`, and the paired dispatch telemetry
tests (`TestLLVMDispatchAppendsLIRProtoFallbackWarning`,
`TestLLVMDispatchSkipsLIRProtoWarningWhenGateOff`) were removed once LIR
Proto became the production LLVM dispatch path. The historical design
...
Design decision (historical): the first Phase-7 slice landed the gate
scaffold without the runner. `internal/llvmgen/lir_proto_gate.go` exposed
`LIRProtoEnvVar` (`OSTY_LLVM_LIR_PROTO`), `LIRProtoSelected()` (env-var
read with the project's standard truthy/falsy rules), and
`ErrLIRProtoNotWired` (a sentinel that said "you asked for LIR Proto;
the Go-side runner that would call into `toolchain/lir_proto.osty` does
not exist yet; falling back to the current path").
...
path it would have chosen (native-owned fast path or MIR-direct).

The next Phase-7 slice was the actual MIR -> LIR Proto -> LLVM text
...
diagnostic fallback when the prototype declines. That runner is now the
steady-state path; the env gate is gone.
