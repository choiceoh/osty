...
**Status (2026-05, PR #1924 / follow-up cleanups):** the interim
`OSTY_LLVM_LIR_PROTO` env gate and `internal/llvmgen/lir_proto_gate.go`
scaffold are **removed**. Production LLVM dispatch always routes MIR
through the native subprocess boundary (`osty-native-llvmgen` →
`osty-native-lirproto` → `toolchain/lir_proto.osty`). There is no
host-side env toggle to opt into LIR Proto anymore; see
`internal/backend/llvm.go::generateLLVMIR` ("no separate host-side env
gate anymore").

Historical note: Phase 7 originally landed a gate-off / gate-on scaffold
(`OSTY_LLVM_LIR_PROTO=1`, `ErrLIRProtoNotWired` warning telemetry, and
`TestLLVMDispatchAppendsLIRProtoFallbackWarning`) so early adopters
could flip without changing default output. Those tests and helpers were
deleted once the subprocess runner became the only path.

Exit criteria (met):

- Default builds use LIR Proto via the managed subprocess.
- Unsupported shapes surface structured diagnostics / skeleton IR; no
  legacy HIR bridge retry.
- Backend dispatch tests pin `llvmDispatchNativeOwned` /
  `llvmDispatchMIRDirect` without referencing the retired env var.

## Phase 8: default-on decision

Deliverables:

- Flip a narrow default-on subset only after Phase 7 subprocess routing
  has been stable (met for production LLVM).
