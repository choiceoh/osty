...
## Phase 7: one-shot wiring behind a gate — **retired (PR #1924)**

The interim `OSTY_LLVM_LIR_PROTO` env gate, `internal/llvmgen/lir_proto_gate.go`,
`LIRProtoSelected()`, and the paired dispatch tests
(`TestLLVMDispatchAppendsLIRProtoFallbackWarning`,
`TestLLVMDispatchSkipsLIRProtoWarningWhenGateOff`) were removed once LIR Proto
became the sole LLVM lowering path. `internal/backend/llvm.go::generateLLVMIR`
now always routes MIR → native-owned subprocess → LIR Proto; there is no
host-side opt-in flag and no `ErrLIRProtoNotWired` fallback scaffold.

Historical note: the first Phase-7 slice intentionally landed only the gate
scaffold without the runner so flipping the env var early stayed safe (gate on
appended a warning and continued through the existing fallback). Subsequent
phases wired the real runner and later deleted the gate entirely.

Exit criteria (met):

- Production LLVM dispatch always uses LIR Proto (no parallel env gate).
- Unsupported shapes surface as structured diagnostics on the MIR-direct path.
- Stale gate plumbing and its unit tests are gone from the tree.
