...
- `OSTY_STDLIB_BODY_LOWER=0` (escape hatch): stdlib body injection off — use for bisect / stage0 bootstrap
- default (unset): stdlib body inject on since PR #1997 — stage0 패턴 매칭 부족 시 `osty_std_json__asString does not match any stage0 pattern` 등 link/emit wall
