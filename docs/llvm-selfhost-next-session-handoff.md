...
- `OSTY_STDLIB_BODY_LOWER` unset (production default ON since PR #1998): stdlib bodies inject into the user module; stage0 fallback may still decline some monomorphized shapes.
- `OSTY_STDLIB_BODY_LOWER=0` (escape hatch): `std.json.parseValue` 등 undefined symbol (link 실패) when injection is off.
- Stage0 path with injection ON may still hit pattern-matching gaps → e.g. `osty_std_json__asString does not match any stage0 pattern`
