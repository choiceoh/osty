...
| **L5** `OSTY_STAGE0_FALLBACK=1` | Go-side stage0 emergency emitter | Per `docs/osty_self_b2_1_audit.md` — **audit** coverage of `toolchain/*.osty` reached **100%** after PR #2023 (`OSTY_STAGE0_AUDIT=1`, `TestStage0ToolchainAudit`). That does **not** mean every monomorphized `install-self` build is link-clean: production paths still need LIR Proto / cross-pkg fixes tracked in `SPEC_GAPS.md`. Stage0 remains invaluable for decline diagnostics when bootstrap is broken. |
...
- L5 (`OSTY_STAGE0_FALLBACK=1`) cannot cover the current toolchain **without**
  the LIR Proto / cross-pkg trajectory closing the remaining build-pass gaps
  (audit-pass ≠ build-pass — see `cmd/osty-native-checker/README.md`).
...
2. **Stage0 unlocks** — same document, batches B2.4–B2.7. Stage0 **audit**
   reached 100% (PR #2023, 2026-05-26); remaining DR2 work is LIR Proto,
   monomorph, and cross-pkg link coverage rather than expanding stage0
   pattern tables from an 11.3% baseline.
...
- **L5** (`stage0`) is the only layer that requires no network and no
  prior binary. Its **audit** coverage is now 100% on `toolchain/*.osty`,
  but full production `osty-self` / `osty-native-checker` link still depends
  on LIR Proto and cross-pkg work — stage0 alone does not close that wall.
  When bootstrap is broken, stage0's decline messages still pinpoint the
  missing MIR shape (`docs/osty_self_b2_1_audit.md` §3). Do not retire
  stage0; its diagnostic value alone justifies keeping the emitter in
