...
| **L5** `OSTY_STAGE0_FALLBACK=1` | Go-side stage0 emergency emitter | Stage0 **audit** of `toolchain/*.osty` reached 100% (PR #1858, `TestStage0ToolchainAudit`). `install-self` can still decline on monomorphized instances and LIR Proto / cross-pkg walls — insufficient for a full production self-host without a cached `osty-self`. |
...
2. **Stage0 unlocks** — same document, batches B2.4–B2.7. Stage0 audit
   coverage reached 100% (PR #1858); remaining work is LIR Proto /
   monomorph / cross-pkg build-pass gaps tracked in
   `docs/llvm-selfhost-plan.md` and `SPEC_GAPS.md`.
...
- **L5** (`stage0`) is the only layer that requires no network and no
  prior binary. Its audit coverage is now 100% of checker-bundle
  functions (PR #1858), but `install-self` still hits monomorph and LIR
  Proto declines in practice. Stage0 remains invaluable for diagnosis:
  when the bootstrap is broken, stage0's decline message tells you
  exactly which MIR shape is missing
  (`docs/osty_self_b2_1_audit.md` §3 enumerates the dominant ones).
  Do not retire stage0 just because production self-host still needs
  `osty-self`; its diagnostic value alone justifies keeping the ~6K lines
  in `internal/backend/stage0/`.
