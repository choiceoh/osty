...
2. **Stage0 unlocks** — same document, batches B2.4–B2.7. Stage0
   **audit** of `toolchain/*.osty` reached **100%** after PR
   [#1858](https://github.com/choiceoh/osty/pull/1858), but
   monomorphized `install-self` / LIR Proto builds can still decline on
   cross-pkg or layout walls tracked in [`SPEC_GAPS.md`](../SPEC_GAPS.md).
   DR2 stage0 work therefore targets **build-pass** gaps, not the
   historical audit-percent shortfall.

Combined, the simplification + targeted stage0 unlock streams aim to
produce a fresh `osty-self` on a single host when every higher layer
(L1–L4) is unavailable. From that point, DR1 applies normally.
