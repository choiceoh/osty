...
```
$ go build -o /tmp/go-checker     ./cmd/osty-native-checker         # Go-built
$ osty build --backend llvm cmd/osty-native-checker/ -o /tmp/llvm-checker   # LLVM-built (이 plan 의 산출물)
$ for fixture in testdata/selfhost_parity/*.request.json; do
    diff <(/tmp/go-checker < $fixture) <(/tmp/llvm-checker < $fixture) || fail
  done
```

위 `osty build cmd/osty-native-checker/` 형태는 **현재 미지원** — `osty build` 는 manifest (`osty.toml`) 가 있는 패키지 디렉토리를 받는다. PR1 에 `cmd/osty-native-checker/osty.toml` 도 같이 추가 (또는 R1 의 대안 모델).
