...
- **Fresh-clone bootstrap is wired** — `just bootstrap` (with `OSTY_STAGE0_FALLBACK=1` baked in) and the per-PR `fresh-clone-source-bootstrap.yml` workflow exercise the stage0 source-bootstrap path when the registry is offline or unreachable. `scripts/verify-self-rebuild` stage1 uses the same fallback to produce `osty-self-1` without a pre-existing `osty-self`.
...
| **P24+** | **superseded for audit %** — PR [#1858](https://github.com/choiceoh/osty/pull/1858) closed the checker-bundle audit at **100.0% (8240/8241)** on 2026-05-17. Remaining work is **build-pass** (LIR Proto / monomorph / cross-pkg), not further audit %. See [`SPEC_GAPS.md`](../SPEC_GAPS.md) `cross-pkg-module-resolution`. | `install-self` / `verify-self-rebuild` |
...
host_osty (Go-built .bin/osty)
    └─ stage1 build: host_osty build toolchain/  →  osty-self-1 (stage0 fallback + offline registry)
...
Stage1 is the only stage that uses the host `osty` LLVM backend with
`OSTY_STAGE0_FALLBACK=1`. Stages 2–3 use the prior stage's `osty-self`
only. External `OSTY_SELF_REBUILD_STAGE{1,2,3}_BIN` overrides are
rejected so the ratchet cannot be short-circuited.

**Distinction from `just verify-selfhost`**: the ratchet exercises the
full MIR → LIR Proto → LLVM pipeline and byte parity across rebuild
generations. `verify-selfhost` runs narrow snapshot parity tests under
`internal/ci` and `internal/runner` only.
