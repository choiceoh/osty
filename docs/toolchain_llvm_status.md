...
| Checker boundary | `internal/check` / `internal/toolchain` | host manages the `osty-native-checker` subprocess (managed artifact under `.osty/toolchain/<ver>/` or `OSTY_NATIVE_CHECKER_BIN`); managed-build failures surface as errors — no production embedded fallback (`SUBPROCESS_SWITCHOVER.md`) |
