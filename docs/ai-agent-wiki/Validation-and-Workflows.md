...
- for LLVM self-host / `osty-self` byte parity, also run
  `just verify-self-rebuild` (or `just backend-loop` when touching backend
  emission). This is **broader** than `verify-selfhost` — see `README.md`
