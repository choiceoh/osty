...
- for full source-compiler chain health, also run `just verify-self-rebuild`
  (or `just verify-self-rebuild-gates` when iterating gates only)
- include `just ci`
...
- LIR Proto subprocess compat env vars (`OSTY_LIRPROTO_*`) default to
  production-safe behavior: source re-lowering is **opt-in** via
  `OSTY_LIRPROTO_SOURCE_COMPAT_MAX_BYTES`; do not set positive values in CI
  unless bisecting legacy seeds (`cmd/osty-native-lirproto/main.go`)
