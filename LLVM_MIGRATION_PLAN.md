...
| ------------------------------------ | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `List<T>` literal + 메서드 lowering  | 🟡 기본 literal/intrinsic 경로 + residual combinator (`enumerate`/`chunked`/`windowed`/`partition`/`zip`) 회귀락 | README status + `TestListResidualHelpersSpecializeCleanly`                                                                                      | residual mut helper (`reverse`/`removeAt`/`sort`) 의 body-injection vs intrinsic dispatch 인터랙션 결정 |
...
| Optional aggregate lowering          | 🟡 MIR-층 struct payload `?` / `?.field` 회귀락 + `recoverFieldType` 가 OptionalType 받아 `<error>` 누설 차단 | `TestLowerOptionalStructPayloadProjectsThroughVariant`, LIR Proto `source_optional_field_chain` parity | `%Struct` load/extract + null phi 의 LLVM 회귀락 (osty-self 필요) + if-let 패턴 바인딩 타입 누설 |
| Generic / interface call lowering    | 🟡 generic method turbofish는 MIR/LLVM 회귀로 잠김; same-package interface boxing/dispatch covered; **cross-pkg interface boxing** for struct→interface assign (e.g. `Err(error.new(...))`) landed PR #2004/#2007 — cross-pkg **vtable dispatch** (step 3.5) still open | `TestLowerGenericOwnerMethodTurbofishUsesMonomorphizedSymbol`, `TestLLVMBackendBinaryRunsGenericOwnerMethodTurbofish`, `TestLLVMBackendBinaryCrossPkgInterfaceBoxingErrConstruct` | cross-pkg vtable injection; generic method turbofish regressions |
| Pattern lowering                     | 🟡 MIR-층 nested binding/destructure 회귀락 완료; LLVM emission 회귀락은 LIR Proto parity 에서                                                | `TestLowerNestedStructBindingPatternKeepsAliasAndLeaves` (IR-direct), `TestLowerNestedStructBindingPatternEndToEnd` (source→MIR), `source_nested_struct_binding` (LIR Proto parity) | LLVM `extractvalue` cascade 회귀락은 native osty-self 환경에서만 가시화 |
...
| `String` / `std.strings` surface     | 🟡 runtime-backed paths expanded; bodied helper 표면은 회귀락만 남음                  | `TestStringsPureBodyHelpersLowerThroughInjection` (split/join/trim/replace/toUpper/repeat/contains/indexOf/splitLines/fields)                   | remaining ABI edge cases on rarely-used helpers; pure body 표면은 회귀락 완료 |
...
**요약**: 현재 `toolchain/*.osty`의 1개 파일만 호스트 런타임 바인딩을 통과하는 **bootstrap-only 어댑터**다. LLVM 셀프-컴파일 경로 밖에 있고, native whole-toolchain LLVM probe에서 자동으로 스킵된다. 이 구조 덕분에 probe가 "셀프호스팅 시 진짜 남는 벽"만 보여준다.
...
세 패턴 모두 LLVM native lowering 없이는 LLVM binary로 self-compile 불가. 명시적 `// bootstrap-only` 주석 마커 대신 FFI 스탠자 존재를 시맨틱 시그널로 사용한다. 새 파일 추가 시 별도 작업 불필요 — `use go` / `use runtime.golegacy` / `use runtime.cihost` 를 쓰면 자동 분류됨.
...
- `selfhost.ResolveStructuredFromRun(*FrontendRun) ResolveResult` — arena-direct native resolve 진입점 (PR #621).
- `selfhost.CheckStructuredFromRun(*FrontendRun) CheckResult` — arena-direct native check 진입점 (PR #623).
