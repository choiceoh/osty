...
CLI 재배선의 resolve + check 축은 **단일 파일 + 단일 패키지** 경로에서 이미 완료됐다. 각 subcommand의 FILE/DIR × legacy/native 조합이 전부 `selfhost.AstbridgeLowerCount` 카운터로 in-process 회귀 테스트 pin됐고, native 경로는 `astLowerPublicFile` 호출을 전혀 트리거하지 않는다.
...
| `osty check FILE` (legacy) | 1 | `TestRunCheckFileLegacyAstbridgeBaseline` |
| `osty check DIR` (legacy, N=2) | 2 | `TestRunCheckPackageLegacyAstbridgeBaseline` |
| `osty typecheck FILE` (legacy) | 1 | `TestRunTypecheckFileLegacyAstbridgeBaseline` |
| `osty lint FILE` (legacy) | 1 | `TestRunLintFileLegacyAstbridgeBaseline` |
| `osty lint DIR` (legacy, N=2) | 2 | `TestRunLintPackageLegacyAstbridgeBaseline` |

양방향 회귀 넷이 갖춰졌기 때문에 다음이 자동으로 잡힌다:
- 레거시 경로 비용 증가 (로더/캐시 변경으로 재lowering 발생)
- 네이티브 경로 비용 누출 (예: fallback이 `EnsureFiles`를 부당하게 발동 → PR #641에서 발견 + 수정된 실제 버그)
