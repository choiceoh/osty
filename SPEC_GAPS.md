...
| **G45** | Golden tests (§11.5) | `#[golden("path", mode="text"|"ast", normalize_diag=…)]` snapshot 비교 (cut F, pre-release amendment 로 4 → 2 modes + flag). text mode 는 BOM 제거 + CRLF/CR→LF 정규화 후 byte 비교(trailing newline 보존). ast mode 는 Osty source 또는 JSON 으로 reparse — JSON 은 key 순서 무시. `normalize_diag = true` flag 가 진단 출력의 Span 차이 흡수 (text mode 와 결합). `osty test --golden` / `--update-golden`. 함수는 reproducible 검증 통과 필수 (`E0444`). | decided |
