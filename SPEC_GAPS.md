...
| **G40** | Sealed construct (§3.4.5) | `#[sealed_construct(name)]` — 명시한 constructor 만 허용. 외부 struct literal / spread update / 직접 mutation / generic deserialize / FFI default 모두 차단. Escape 는 같은 어노테이션의 `escape=` 옵션: `escape=trusted` (stdlib/internal, `reason="..."` 필수) / `escape=test` (test profile only). parse-don't-validate 패턴을 언어 primitive 로. v0.6 baseline 에서 stdlib `Email` / `Url` / `Path` / `SqlIdent` / `Duration` / `Uuid` 등 sealed 화. **Pre-release amendment (cut H)**: 별개 `#[trusted_construct]` / `#[test_construct]` 두 어노테이션을 위 `escape=trusted/test` 키워드로 흡수 — annotation 카운트 -2. | decided (cut H amended) |
