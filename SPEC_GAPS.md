...
5. **`toolchain/resolve.osty:499` — use-decl 처리 시 alias 만 `"package"` kind 의 symbol 로 등록, full import path 미저장**. 즉 `use toolchain.check as tc` 시 `tc` 는 알려지지만 어떤 module 인지 추적 없음 → `tc.frontInvalidTypeRepr()` 호출 시 method-on-package 인지 못 하고 method-on-type 으로 fallback → E0703. 진짜 fix = `SelfSymbol` 에 import-path 필드 추가 + module-scoped lookup arm (`elab.osty` 의 method-call dispatch 에 새 분기). 추정 ~150 LOC + 다수 사이트 회귀 검증.
   ...
   **2026-05-17 R3 trajectory brick 진척** (PR #1873 + #1874 + brick 3a 시도):
   ...
   - **Brick 2 (PR #1874)**: `Int__abs/min/max/clamp/signum` C runtime wrapper + `rewriteStdlibSymbolToRuntime` 매핑 + `mangleMethodSymbol` filter through rewrite. source-bootstrap LIR Proto path 의 link wall 해소 — osty-tests 129 tests passed (이전 51/51 fail). stage0 의 inline expansion 잃지만 function call overhead 무시 가능.
   - **Brick 3a 시도 (이 PR, revert 됨)**: `toolchain/lir_proto.osty` 에 inline expansion 추가 — `LirInstrSelect` enum + `lirSelect` constructor + `lirTryLowerMirIntMethodIntrinsic` 함수. osty-self 빌드 성공하나 `--selfhost-doctor` SEGFAULT (exit 139), osty-tests 51/51 fail 회귀. root cause 후보 3개 (enum ordinal change / args OOB / monomorph cover gap) — 다음 fresh session 의 measurement spike 로 식별. 상세: `docs/llvm-selfhost-plan-r3-brick-3a-attempt.md`.
