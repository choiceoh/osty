...
개선점(G20-G34) 을 v0.5 에서 일괄 결정했다. v0.5 는 **최종 freeze**
릴리스이므로 이후 새 gap 번호 부여는 금지된다 — 문법 / prelude /
§14 excluded / stdlib 공개 서명 변경은 새 edition (v1.0+) 에서만
가능하다.
...
**Freeze 이후 정책.** 새 G 번호 부여 금지. 언어 surface 변경 제안은
모두 edition proposal (v1.0+) 로만 접수. 버그 수정 / 명확화 /
성능 최적화 / 의미 중립 변경은 G 번호 없이 일반 이슈 트래커에서 처리.
...
결정해서 언어 surface 를 영구 잠그는 릴리스다. 모두 additive — v0.4
프로그램은 바이트 단위 변경 없이 컴파일된다.
...
| **G21** | Default argument literal definition extension | `DefaultLiteral` 집합이 (a) 필드가 모두 literal 인 struct literal 과 (b) `const fn` 호출 반환값을 포함. 평가 시점은 기존 규칙 (each call site) 유지. | decided |
...
이 표는 v0.5 freeze 에 올린 최종 결정안이다. 원칙 네 가지:
...
| **G21** | `DefaultLiteral` 을 재귀적으로 정의해서 fields 가 literal 인 struct literal 과 `const fn` 호출을 포함한다. | 팩토리 함수로 우회해야 했던 default 인자 케이스를 직접 작성하게 한다. "각 call site 에서 재평가" 규칙은 그대로 유지. | default expr 파서에 struct literal / `const fn` 호출 허용. checker 가 `isDefaultLiteral` 을 재귀 정의. `const fn` 본문이 purity check 를 통과해야 default 위치 사용 가능. |
...
| 항목 | 분류 | 처리 |
|---|---|---|
| generic type parameter defaults (`struct Pair<T, U = T>`) | excluded feature | v0.5 영구 제외. 재오픈하려면 새 edition 제안. |
