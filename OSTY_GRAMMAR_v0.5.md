...
**v0.5 허용 집합** (컴파일러 고정 세트, 사용자 정의 불가):
...
> **A5.2 — vectorize 는 기본 ON.** 모든 함수의 `for` 루프에 자동으로
> `!llvm.loop.vectorize.enable, i1 true` metadata 가 붙고 per-iteration GC
> safepoint poll 이 스킵된다. 어노테이션 없이 vectorize 가 적용되며,
> `#[vectorize(scalable, predicate, width = N)]` 은 strategy tuning 만,
> `#[no_vectorize]` 는 opt-out 이다. 자세한 contract 는 `LANG_SPEC_v0.5/03-declarations.md` §3.8.3 참조.
