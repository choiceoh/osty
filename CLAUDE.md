...
- 공개 백엔드: LLVM만. 부트스트랩 Osty→Go 트랜스파일러는 제거됨. `internal/selfhost/generated.go`(68k 줄)는 커밋된 시드 산출물로 남아 있으며 더 이상 재생성 경로가 없다 — `toolchain/*.osty` 수정은 LLVM 셀프호스팅 경로로만 반영된다
- Go 1.26+ (부트스트랩 빌드용)
...
- 새 작업은 메인 checkout에서 바로 수정하지 말고, 먼저 `cd "$(aitask <slug>)"`로 격리된 task worktree를 만든다. 현재 상태 확인은 `aistatus`.
- task worktree에서의 저장은 `aisave "<type: summary>"`를 우선 사용한다. 원격 브랜치까지 올릴 때만 `aisave --push ...`.
- 셸 선택이 가능하면, hook이 많은 interactive `zsh`보다 가벼운 `bash` login shell을 우선 사용한다.
- 일반 텍스트 검색은 `rg`, 파일 탐색은 `fd`.
- 디렉토리 구조 확인은 `eza`, 빠른 파일 미리보기는 `bat`.
- JSON은 `jq`, YAML/TOML 류 설정 편집은 `yq`.
- 반복 가능한 CLI 성능 비교는 `hyperfine`.
- 저장-재실행 루프는 `watchexec` 기반 흐름을 우선 사용.
- 구조적 검색/리팩터링이 필요하면 `ast-grep`을 우선 사용. 단순 텍스트 검색은 `rg`.
- shell 스크립트를 작성/수정했으면 `shfmt -w` 후 `shellcheck`까지 실행.
- 큰 Go 테스트 러닝은 `gotestsum`을 우선 사용해 실패 요약을 읽기 쉽게 유지.
