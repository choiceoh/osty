...
| 4 | function emission | 1642–2384 | ~18 | MEDIUM | Partial — pure post-process leaves ported (`mirIsMemoryAccessLine`, `mirTagParallelAccesses`); state-bearing pieces (`emitFunction`, `emitAllocaPreamble`, `emitVectorListPreamble`, `snapshotVectorListLocal`, `emitVectorListFastLoad`/`Store`, `mirBlockSuccessors`, `mirLocalIsMutatedIn`, `mirLocalMutationReachableFrom`, `snapshotValidInCurBlock`, `invalidateStaleVectorListSnapshot`) blocked on Phase B fnBuf/declares mirror + MIR-type Osty surface |
