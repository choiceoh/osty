...
`PrepareEntry` always injects Osty-bodied stdlib methods (for example `collections.osty` `flatMap` / `zip`) into the user module before MIR so link resolves bodied helpers instead of stopping at missing `osty_rt_*` symbols. The former `OSTY_STDLIB_BODY_LOWER` rollout gate was removed after the default-on path proved stable through CI.
