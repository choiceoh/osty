...
**LLVM backend** ([`internal/backend/entry.go`](./internal/backend/entry.go)):

`PrepareEntry` always injects Osty-bodied stdlib methods from `internal/stdlib/modules/` into the user module before MIR lowering so link resolves bodied helpers (for example `collections.osty` `flatMap` / `zip`) instead of stopping at missing `osty_rt_*` symbols.
