...
**LLVM backend / stdlib lowering** ([`internal/backend/entry.go`](./internal/backend/entry.go)): `PrepareEntry` always injects Osty-bodied stdlib methods (for example `collections.osty` `flatMap` / `zip`) into the user module. The former `OSTY_STDLIB_BODY_LOWER` rollout gate was removed after the default-on path proved stable through CI.
