...
const (
	...
	// CodeSinceBadFormat: the `since` keyword argument on
	// `#[stability(...)]` did not receive exactly one SemVer-shaped
	// string literal. Accepted shapes are `"X.Y"`, `"X.Y.Z"`, and
	// `"X.Y.Z-pre"` where each `X` / `Y` / `Z` is an ASCII digit run and
	// the optional pre-release tail is `[A-Za-z0-9.-]+`. Interpolated
	// strings, integer literals, identifiers, prefixes such as `"v0.6"`,
	// and wildcard tails such as `"0.6.x"` are rejected. (Pre-release v0.6
	// cut C: standalone `#[since]` was unified into `#[stability(since=)]`;
	// the format gate moved with it.)
	//
	// Spec: v0.6 §3.14.1
	// Fix: pass `#[stability(level = ..., since = "X.Y")]` with a string
	// literal whose contents match `^[0-9]+\.[0-9]+(\.[0-9]+)?(-[A-Za-z0-9.-]+)?$`
	// (e.g. `"0.6"`, `"1.0.0"`, `"2.0.0-rc.1"`).
