...
func TestRunGenLLVMBackend(t *testing.T) {
	r := RunWithConfig([]byte(cleanSrc), nil, Config{
		RunGen:        true,
		GenBackend:    backend.NameLLVM,
		GenEmit:       backend.EmitLLVMIR,
		GenSourcePath: "/tmp/pipeline.osty",
	})
	if len(r.Stages) != 6 {
		t.Fatalf("Stages: got %d, want 6 (lex/parse/resolve/check/lint/gen); names=%v",
			len(r.Stages), stageNames(r.Stages))
	...
	text := string(r.GenBytes)
	for _, want := range []string{
		"; Code generated by osty LLVM backend. DO NOT EDIT.",
		`source_filename = "/tmp/pipeline.osty"`,
		"define i32 @main()",
		"call i32 (ptr, ...) @printf",
	} {
		if !strings.Contains(text, want) {
			t.Fatalf("LLVM pipeline gen missing %q:\n%s", want, text)
		}
	}
	if !strings.Contains(r.Stages[5].Output, "LLVM IR") {
		t.Fatalf("gen stage output = %q, want LLVM IR label", r.Stages[5].Output)
	}
}
