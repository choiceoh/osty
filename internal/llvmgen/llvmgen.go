...
func generateASTFile(file *ast.File, opts Options) ([]byte, error) {
	...
	if len(file.Stmts) > 0 {
		...
		g.stdLogAliases = collectStdLogAliases(file)
		g.stdEncodingAliases = collectStdEncodingAliases(file)
		...
		g.stdKeychainAliases = collectStdKeychainAliases(file)
		g.stdSecretsAliases = collectStdSecretsAliases(file)
		g.stdMathAliases = collectStdMathAliases(file)
