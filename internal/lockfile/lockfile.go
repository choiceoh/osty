...
//
// Sort + emit policy lives in toolchain/lockfile_policy.osty
// (`marshalLockfile`); this wrapper marshals the host types into
// the policy spec.
func Marshal(l *Lock) []byte {
	specs := make([]runner.LockPackageSpec, 0, len(l.Packages))
	for _, p := range l.Packages {
		deps := make([]runner.LockDepSpec, 0, len(p.Dependencies))
		for _, d := range p.Dependencies {
			deps = append(deps, runner.LockDepSpec{
				Name:    d.Name,
				Version: d.Version,
				Source:  d.Source,
			})
		}
		specs = append(specs, runner.LockPackageSpec{
			Name:         p.Name,
			Version:      p.Version,
			Source:       p.Source,
			Checksum:     p.Checksum,
	...
	return []byte(runner.MarshalLockfile(l.Version, specs))
}
