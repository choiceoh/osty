...
func (p *Parser) parseUseDecl() *ast.UseDecl {
	...
	if strings.HasPrefix(u.RawPath, "runtime.") {
		u.IsRuntimeFFI = true
		u.RuntimePath = u.RawPath
		if p.eat(token.LBRACE) {
			p.skipNewlines()
			for !p.at(token.RBRACE) && !p.at(token.EOF) {
				if p.peek().Kind != token.FN {
					p.errorf(p.peek().Pos,
						"`use runtime` body may only contain `fn` declarations, got %s",
						p.peek().Kind)
					p.advance()
					p.skipNewlines()
					continue
				...
				fd := p.parseFnDecl()
				if fd.Body != nil {
					p.errorf(fd.Body.PosV, "`use runtime` function declarations may not have a body")
					fd.Body = nil
				}
				u.GoBody = append(u.GoBody, fd)
				p.skipNewlines()
