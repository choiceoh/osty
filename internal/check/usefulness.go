...
// This file implements a simplified but faithful version of the
// Maranget (2007) usefulness algorithm for match exhaustiveness. A
// "pattern matrix" has one row per arm and one column per scrutinee
// component (starts at 1; recursive calls grow the column count as
// constructors are specialized open). The question we answer is:
//
//     Does the matrix cover every value of its column-type list?
//
// For types whose constructor space is NOT finite-enumerable (Int,
// String, Char, Float, Bytes), coverage requires a column-0 wildcard.
// For types that ARE enumerable (Bool, Option, Result, user enums,
// tuples, structs, Unit) we specialize the matrix by each constructor
// and check recursively.

// patRow is an arm's full scrutinee pattern, with a guard flag.
// Guarded arms are skipped by isExhaustive — per §4.3.2 they cannot
// contribute to the coverage proof.
type patRow struct {
	pat     ast.Pattern
	guarded bool
}

// isExhaustive is the entry point: the flat list of match arms
// together with the scrutinee's type.
func (c *checker) isExhaustive(arms []patRow, t types.Type) bool {
	rows := make([][]ast.Pattern, 0, len(arms))
	for _, a := range arms {
		if a.guarded {
			continue
		}
		// Or-pattern splits into multiple rows: each alternative
		// gets its own row so exhaustiveness sees them as
		// independent contributors.
		for _, alt := range splitOr(a.pat) {
			rows = append(rows, []ast.Pattern{alt})
		}
	}
	return c.coversMatrix(rows, []types.Type{t})
}

// coversMatrix is the core recursive relation.
func (c *checker) coversMatrix(rows [][]ast.Pattern, cols []types.Type) bool {
	if len(cols) == 0 {
		// Matrix has no columns — covered iff any row remains (a row
		// without columns represents an arm that already matched all
		// remaining columns via wildcards).
		return len(rows) > 0
	}
	t0 := cols[0]
	tail := cols[1:]

	// Check whether any row has a catch-all at column 0 — that single
	// row collapses the column into its default matrix D(P).
	hasWild := false
	for _, r := range rows {
	...
	if !enumerable {
		// The column's type has an open constructor space. Coverage
		// requires SOME row to wildcard this column, then the tail is
		// recursed with the default matrix.
		if !hasWild {
			return false
		}
		return c.coversMatrix(c.defaultRows(rows), tail)
	}

	// Finite ctor space: each ctor must be covered. For each ctor we
	// specialize the matrix (S(c, P)) and recurse on the expanded
	// column list (ctor's arg types prepended to tail).
	for _, ct := range ctors {
		spec := c.specialize(rows, ct)
		newCols := append(append([]types.Type{}, ct.argTypes...), tail...)
		if !c.coversMatrix(spec, newCols) {
			return false
		}
	}
	return true
