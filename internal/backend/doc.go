...
// 64-73 value/control-flow smoke expansion, is authored in Osty selfhost-core.
// `Float32`/`Float64` policy is deferred. LLVM binary emission also links a
// local backend runtime ABI object for the `osty.gc.*` surface from the backend
// runtime directory at native link time rather than relying on Go GC parity.
