...
// TestLLVMBackendEmitStringsCompareTelemetry logs the end-to-end outcome
// for strings.compare when stdlib bodies are always injected (legacy
// `OSTY_STDLIB_BODY_LOWER` gate removed).
func TestLLVMBackendEmitStringsCompareTelemetry(t *testing.T) {
...
`)
	...
	if err != nil {
		t.Logf("strings.compare top err: %v", err)
		return
	}
	t.Logf("strings.compare succeeded end-to-end")
}

// TestPrepareEntryInjectsStdlibStringsCompare verifies injection runs in
// the real PrepareEntry path by inspecting entry.IR directly — bypasses
// the llvmgen/AST legacy bridge so we see the HIR-level result without any
// downstream transformations.
func TestPrepareEntryInjectsStdlibStringsCompare(t *testing.T) {
...
// `Ident{Kind: IdentGlobal, Name: "graphemeBreakCR"}` — every reference
// must resolve to the mangled name.
