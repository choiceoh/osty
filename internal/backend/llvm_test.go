...
func TestLLVMBackendIRElidesMapKeysSortedLenChain(t *testing.T) {
	requireRealLLVMEmission(t)
	// With stdlib body injection, `keys()` / `sorted()` lower through
	// their Osty bodies; the IR optimizer still folds the chain to a
	// single map length read (no standalone keys/sorted runtime calls).
	backend := LLVMBackend{}
