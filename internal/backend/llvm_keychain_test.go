...
func TestLLVMBackendStdKeychainApiKeyWrapperLowers(t *testing.T) {
	// Assert the `keychain.getApiKey` wrapper surfaces in LLVM IR as a
	// cross-pkg call (not only the lower-level `keychain.get` intrinsic).
	// The Ok arm + a sentinel match-all on Err avoids virtual dispatch
	// on the cross-pkg Error interface in the Err branch.
