...
func TestLLVMBackendStdKeychainApiKeyWrapperLowers(t *testing.T) {
	// Assert the `getApiKey` wrapper call shape survives lowering:
	// `requireName("provider", ...)` + delegation to
	// `get(defaultApiKeyService, ...)`. The Ok arm + a sentinel
	// match-all on Err is enough to drive the wrapper into the IR.
