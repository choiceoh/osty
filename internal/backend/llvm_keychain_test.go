...
func TestLLVMBackendStdKeychainApiKeyWrapperLowers(t *testing.T) {
	// Asserts the `getApiKey` wrapper surfaces in generated IR as a
	// cross-pkg call. Keep the call shape so the test covers the
	// WRAPPER path (`requireName("provider", ...)` + delegation to
	// `get(defaultApiKeyService, ...)`) rather than a direct
	// `keychain.get` call. The Ok arm + a sentinel match-all on Err
	// avoids virtual dispatch on the cross-pkg Error interface.
