...
| Deterministic capability inference | **planned** | Pre-release amendment (cut D): `#[reproducible_capability]` annotation + `E0783` removed in favour of compiler-side inference — an interface is classified as reproducible iff every method carries `#[reproducible]` / `#[pure]`. Surfacing happens at the call site through the existing `E0784` gate. |
