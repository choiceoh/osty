...
| `#[ambient(...)]` annotation | **partial** | active checker gate now rejects non-entry usage (`E0780`) and unknown canonical names (`E0781`). Positive/negative spec corpus plus `examples/v06_ambient_boundaries` pin the boundary; auto-forward/desugar remains follow-up. |
...
| Item | Status | Notes |
|---|---|---|
| `#[reproducible(scope = "run"|"target"|"portable")]` | **planned** (G39, Phase 3) | capability 기반 sound |
