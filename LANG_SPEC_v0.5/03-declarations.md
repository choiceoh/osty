...
| `#[vectorize]` | top-level `fn` declarations, struct/enum methods | **Default-on** — no annotation needed for the hint. Bare `#[vectorize]` is a no-op; `#[vectorize(scalable, predicate, width = N)]` refines strategy (§3.8.3) |
| `#[no_vectorize]` | top-level `fn` declarations, struct/enum methods | Opt out of the default vectorize hint. Restores per-iteration safepoint polls (A5.2) |
| `#[parallel]` | top-level `fn` declarations, struct/enum methods | Hint: every load/store in the body tagged with `!llvm.access.group`, every loop's metadata references it via `llvm.loop.parallel_accesses` to bypass alias analysis (A6) |
| `#[unroll]` | top-level `fn` declarations, struct/enum methods | Hint: `llvm.loop.unroll.enable` on every loop in the body; `#[unroll(count = N)]` forces a specific unroll factor (A7) |
| `#[inline]` / `#[inline(always)]` / `#[inline(never)]` | top-level `fn` declarations, struct/enum methods | LLVM `inlinehint` / `alwaysinline` / `noinline` fn attribute (A8) |
| `#[hot]` / `#[cold]` | top-level `fn` declarations, struct/enum methods | LLVM `hot` / `cold` fn attribute + `.text.hot` / `.text.unlikely` section placement (A9) |
| `#[target_feature(f1, f2, ...)]` | top-level `fn` declarations, struct/enum methods | LLVM `"target-features"="+f1,+f2"` fn attribute — per-function CPU feature override (A10) |
| `#[noalias]` / `#[noalias(p1, p2)]` | top-level `fn` declarations, struct/enum methods | Promise pointer params do not alias — emits LLVM `noalias` param attr (A11) |
| `#[pure]` | top-level `fn` declarations, struct/enum methods | Assert no side effects — emits LLVM `readnone` fn attr, unlocks CSE/hoisting (A13) |
...
**A5.2: vectorize is the default.** Every function's user-
...
Valid on top-level `fn` declarations and on struct/enum methods.
Status: **A8**. Controls the LLVM inliner's decision for the
annotated function.
...
the rest of the annotation set. Unknown sub-flags are `E0739`.
...
methods. Status: **A9**. Control the LLVM frequency attributes
...
**A10**. Overrides the LLVM backend's target-feature baseline
...
attribute (positive enable only). Duplicate names are
...
Runtime dispatch is not built in; it belongs in the user's
...
Valid on top-level `fn` declarations and on struct/enum methods.
Status: **A11**. Promises pointer-typed parameters do not alias.
...
Valid on top-level `fn` declarations and on struct/enum methods.
Bare flag. Status: **A13** (lenient — the compiler trusts the
...
**Soundness is the programmer's responsibility.** The compiler
