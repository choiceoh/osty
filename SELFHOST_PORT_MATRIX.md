...
| **Builder auto-derive** | `internal/check/builder_desugar.go` | `toolchain/builder_policy.osty` + `toolchain/hir_lower.osty` | E0774 | **partial** | 2026-04-24: HIR 측 derive-policy 를 `builder_policy.osty` 로 분리. Go desugar 도 same-rule mirror 로 `custom builder()` override + private-no-default 비유도 struct 를 더 이상 rewrite 하지 않음. **잔여**: 체인 rewrite 자체와 `toBuilder()` receiver recovery 를 selfhost bridge 뒤로 이동 |
...
3. **Builder auto-derive 단일화** — 2026-04-24 기준 derive-policy shared source (`toolchain/builder_policy.osty`) 는 분리 완료. 남은 핵심은 Go AST 레벨 chain rewrite 를 selfhost 경로로 흡수하는 것. `toBuilder()` receiver type recovery 를 옮기려면 scope 정보 필요 → resolver `#[cfg]` 포팅 이후 진행.
