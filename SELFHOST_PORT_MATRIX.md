...
| **Intrinsic body support** | `selfhost.IntrinsicBodyDiagsForSource` (wrapper, check.go:208/285 에서 호출) | `toolchain/check_gates.osty:63` + `selfhostIntrinsicBodyDiagnosticsFromArena` | E0773 | **ported** | 2026-04-23 Go 레거시 `intrinsic_body.go` (93 LOC) + test 제거. 모든 경로가 Osty arena gate 를 single source of truth 로 사용 — native CheckPackageStructured 내부 호출 + 비-native Go 경로는 public `selfhost.IntrinsicBodyDiagsForSource(src, path)` 래퍼 호출. 더이상 silent diverge 불가능 |
