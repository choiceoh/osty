...
| **Privilege system** | `internal/check/privilege.go` (reference only) | `toolchain/check_gates.osty:376` | E0770 | **ported** | 2026-04-23 `check.go` 의 호출부 제거. Osty 의 `runCheckGates` 가 authoritative; Go 함수는 reference impl + 단위 테스트용으로 유지. parity 는 `gates_diff_test.go::TestGatesCrossSideParity` |
| **POD shape analysis** | `internal/check/podshape.go` (reference only) | `toolchain/check_gates.osty:146` | E0771 | **ported** | 2026-04-23 same consolidation |
| **Noalloc analysis** | `internal/check/noalloc.go` (reference only) | `toolchain/check_gates.osty:275` | E0772 | **ported** | 2026-04-23 consolidation (PR #770). 2026-04-23 추가: `noalloc_test.go` 의 19 fixture 가 이제 `check.File` full pipeline 경유 — Osty `runNoAllocGate` 가 직접 source. `rawptr_test` / `runtime_fixture_test` 가 여전히 Go-side `runNoAllocChecks` 를 호출해 `noalloc.go` 는 보존. 두 파일 추가 마이그레이션 후 전면 삭제 가능 |
