...
> 기준일: 2026-04-26 (post-rebase audit, HEAD = #921). 이전 2026-04-24 스냅샷
> 이후 80 커밋이 들어왔다. 코드 실측 기준: `--legacy` check/typecheck escape
...
> 토론체인 MIR 파이프라인은 커버하지 않음). 단, **`osty check toolchain` smoke
> 와 merged native MIR backend gate 는 현재 실패 상태**다 — 자세한 내용은 아래
> "2026-04-26 — 현재 first-walls" 섹션 참조.

## 2026-04-26 — 현재 first-walls (post-rebase 실측)

`.bin/osty check toolchain` 직접 실행 시 **11 type errors**:

- `E0702 no field 'typeName'` × 9 — [toolchain/inspect.osty](toolchain/inspect.osty)
  (line 503, 525, 547, 681) + [toolchain/lint.osty:2884](toolchain/lint.osty:2884).
  PR #892 (`refactor(selfhost): replace string-based typeName round-trip with
  structured TypeRepr`)이 `FrontCheckedNode` / `FrontCheckedBinding` /
  `FrontCheckedSymbol` / `FrontCheckedInstantiation` 의 `typeName: String`
  필드를 `typeRepr: FrontTypeRepr` 로 갈았는데 `inspect.osty` / `lint.osty`
  소비자 마이그레이션이 누락됨.
- `E0708 missing field 'valueText' / 'targetLabel'` × 2 —
  [toolchain/mir.osty:895](toolchain/mir.osty:895). 같은 패키지에
  **`pub struct MirSwitchCase` 가 두 번 선언**됨:
  [toolchain/mir.osty:888](toolchain/mir.osty:888) (`value/target/label`)
  과 [toolchain/mir_generator.osty:2185](toolchain/mir_generator.osty:2185)
  (`valueText/targetLabel`). §6/§9 line-builder 포팅 (#891–#915 시리즈)
  중에 들어온 충돌. 둘 중 한 쪽을 rename 하거나 한 쪽으로 통일 필요.

`go test ./internal/llvmgen -run TestProbe…` 머지 probe 결과:

| 테스트 | 시간 | First wall |
|---|---|---|
| `TestProbeWholeToolchainMerged` (AST-only) | 2.4s | `LLVM010 source-layout: duplicate struct "MirSwitchCase"` |
| `TestProbeNativeToolchainMerged` (AST, ci.osty 제외) | 2.3s | 동일 |
| `TestProbeNativeToolchainMergedMIR` (full MIR) | 279s | `LLVM000 unsupported-source: unsupported local type <error> in tyToRepr (local _scrut id=7; written by assign at 216:11 (src=UseRV))` (12 checker diag 발화 후) |
| `TestNativeToolchainMergedMIRErrTypeFloor` | 274s | **FAIL** — 474 ErrType locals (floor=40, 12× 초과) |
| `TestNativeToolchainMergedMIRPipelineIsClean` | 300s+ | **TIMEOUT** — `internal/check/host_boundary.go:930` `selfhostSpanIndex.scopeFor` 가 cache miss 시 모든 span 선형 스캔 → 머지 토론체인 규모에서 사실상 O(N²) |

`scopeFor` 의 O(N²) 가 enforcing 테스트에서 5분 timeout 을 내고, 같은 경로의
info-only probe 가 ~280s 로 끝나는 차이는 후자의 bookkeeping 이 더 가볍기
때문 (overlay 생성을 거치지 않음). `LLVM010` / `LLVM000` 가 보이는 이유는 이
경로뿐이다.

이 4 종이 현재 self-host critical path 의 실제 blocker 이다. 이전
revision 이 인용한 `TestGoGenerateSelfhostLeavesGeneratedArtifactsClean`
red light 는 PR #854 (Osty→Go bootstrap transpiler 폐기) 시점에 테스트
자체가 제거됐으며, 더 이상 존재하지 않는다. `LLVM011 CheckFnSig.hasReceiver
default value` 와 `LLVM000 SelfDocDecl indirect call` 도 이전 트리 상태에
대한 관측이며 현재 first-wall 이 아니다.
