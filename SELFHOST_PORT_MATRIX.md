...
| **Workspace cycle detection** | `internal/resolve/workspace.go:468` | — | E0506 | **go-only** | `detectCycles()` over import graph |
...
3. **Workspace cycle detection** (E0506) — import DAG 순환 감지. graph traversal 자체는 단순하지만, Osty 쪽 workspace state 모델이 없어 신규 구조물 필요. Partial merge 이후 packaging 파이프라인과 함께.
...
| Annotation semantic validation | `(hostboundary)` | `toolchain/hir_lower.osty:324` | — | **partial** | `#[inline]`/`#[noalias]`/`#[target_feature]` 착륙, `#[vectorize]`/`#[parallel]`/`#[hot]`/`#[pure]` 일부만 |
...
- **Privilege/POD/Noalloc 가 양쪽 독립 실행 중** — 현재도 조용히 diverge 가능. 포팅 전에 **크로스사이드 diff 테스트** 부터 (같은 입력 → Go 결과 vs Osty 결과 비교) 장착. diverge 발견되면 포팅이 "fix-and-port" 가 되어 스코프 터짐.
