...
| 숫자 리터럴 다형성 | `(hostboundary)` | `toolchain/elab.osty:266,279,290` | — | partial (확장 2026-05-16) | 기대 타입 기반 narrowing — IntLit / FloatLit 직접 + `elabCheckBinary` 가 BoAdd/Sub/Mul/Div/Mod 의 양 피연산자에 expected 전파 (`let x: Float64 = 1 + 2` → 1, 2 둘 다 Float64 직접 채택). 비-arith / 비-numeric expected 는 infer→subtype fallback |
...
| Closure annotation requirement | `(hostboundary)` | `toolchain/elab.osty::elabInferClosure` (line ~3025) | E0752 | partial (test coverage 추가 2026-05-16) | param seeding + 무-context fallback 정상 emit 확인. `internal/check/closure_annotation_test.go` 4 케이스 (unconstrained / mixed-annotation / expected-seeds-param / fully-annotated) + spec corpus waiver. "partial" 잔여: per-param 명시 annotation vs expected fn 타입 mismatch 진단은 closure-level mismatch 로만 노출 (per-param 위치 강조 다이앵 없음) |
