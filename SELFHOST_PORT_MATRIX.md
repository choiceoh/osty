...
| **Inspect / per-node 기록** | `internal/check/inspect.go` (647 LOC — 2026-04-24 실측) | `toolchain/inspect.osty` (v1.5 — `typedNodes` + `symbols` + `bindings` 기반 워커) + `toolchain/inspect_test.osty` | — | **partial** | 2026-04-24 **v1 (#810)** 착륙: expression-level 레코드 19 종. **v1.5 후속** 착륙: decl-level 레코드 (FN-DECL / LET / STRUCT-DECL / ENUM-DECL / INTERFACE-DECL / TYPE-ALIAS / USE / FIELD-DECL / VARIANT-DECL) 를 `FrontCheckResult.symbols` 에서 뽑고, pattern binding (BIND) 를 `FrontCheckResult.bindings` 에서 뽑아 함께 start-offset 정렬 반환. Go reference 대비 STRUCT-DECL 이하는 strict super-set (IDE hint 확장성). **남은 갭**: (1) **Local `let` statement** (블록 내 `let`) 는 self-host 체커가 symbols 에 안 넣고 walker 내부 상태로만 들고있어 여전히 누락 — AST re-walk 또는 symbols 엔트리 추가 필요. (2) `hintName` 은 여전히 빈 문자열 — top-down 힌트 재실행은 `List<T>` / tuple / `Map<K,V>` / `fn(..) -> R` 시그니처 파서를 `typeName: String` 위에 얹어야 함. (3) `notes` (generic instantiation, "method call" 마커) 는 expression 레코드에서 아직 비어있음. (4) Go 브리지 (`selfhost.InspectFromRun`) 는 `bundle.toolchainCheckerFiles` 업데이트 + `just build-all` 로 `internal/selfhost/generated.go` 재생성 필요 |
