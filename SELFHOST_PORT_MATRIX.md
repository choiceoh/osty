...
> 기준일: 2026-04-24. 코드 실측 기준: `--legacy` check/typecheck escape hatch 는
> 제거됐고, `--native` 는 backwards-compat no-op 이다. `just front`,
> `just verify-selfhost`, CLI astbridge-free 회귀 테스트는 통과. 단,
> `go generate ./internal/selfhost` 고정점과 merged native MIR backend gate 는
> 현재 실패 상태다.

## 2026-04-24 — Phase 1c.5 code state
...
- `TestGoGenerateSelfhostLeavesGeneratedArtifactsClean` — **현재 실패**.
  `toolchain/check.osty` 의 `std.strings` alias 추가분 (`Contains`, `Index`,
  `HasPrefix` 등)이 `internal/selfhost/generated.go` 에 반영되지 않았다.
...
- **Phase 2 (checker 활성화)**: inspect → annotation 완성 → defer → worker pool.
