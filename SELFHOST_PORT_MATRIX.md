...
| **Inspect / per-node 기록** | `internal/check/inspect.go` (647 LOC — 2026-04-24 실측) | `toolchain/inspect.osty` + `toolchain/inspect_hint.osty` + tests | — | **partial** | 2026-04-24 점진 착륙: **v1 (#810)** expression 19 종 → **v1.5 (#811)** decl + pattern binding → **v1.6 (#813)** `AstNLet` ident pattern 을 LET 로 승격 → **v1.7 (#817)** destructuring let 까지 LET → **v1.8 (#818)** 타입 문자열 파서 유틸 → **v1.9** `inspectBuildHintsByNode` 가 파서 유틸을 엮어 List/Tuple/Map entry/Call arg 4 가지 parent shape 에서 `hintName` 을 직접 자식 레코드에 채움 (dense NodeID-indexed List 로 O(1) lookup). **남은 갭**: (1) Identity-like / nested 전파 (Block tail, If then/else, Paren inner, Closure body) — 현재는 직접 부모 타입만 보므로 chain 내부로 hint 가 안 흐름. AST recursion walker 로 완성 예정. (2) 선언 annotation 기반 hint (let 선언 타입 / fn return 타입 → RHS/body) — typedNodes 에는 annotation 이 없고 symbols 에서 따로 풀어야 함. (3) Expression `notes` (generic instantiation / method-call 마커) 비어있음. (4) Go 브리지 (`selfhost.InspectFromRun`) — `bundle.toolchainCheckerFiles` + `generated.go` 재생성 필요 |
