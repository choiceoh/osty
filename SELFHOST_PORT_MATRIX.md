...
- **1c.4** ⏳ `check.File(file, res, opts)` 시그니처 제거. 모든 call site 가
  ...
  `noalloc`) 은 `check_gates.osty` 로 이식이 완료된 시점에서 삭제.
  builder auto-derive prepass 는 selfhost checker + IR on-demand lowering
  착륙 후 삭제.
  ...
  **2026-04-24 스코프 검증** (grep 기반 실측):
  ...
  - `check.go` 432 (File/Package/Workspace entry 3종)
