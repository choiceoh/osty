...
| **Inspect / per-node 기록** | `internal/check/inspect.go` (647 LOC — 2026-04-24 실측) | `toolchain/inspect.osty` (v1.7 — AST-aware walker + destructuring) + `toolchain/inspect_test.osty` | — | **partial** | 2026-04-24 **v1 (#810)** → **v1.5 (#811)** → **v1.6 (#813)** → **v1.7** 점진 착륙. v1: expression-level 19 종. v1.5: decl-level + pattern binding. v1.6: `inspectFromAstAndCheck` 가 `AstNLet` 의 ident pattern 에서 binding 레코드의 rule 을 `BIND` → `LET` 로 승격. v1.7: `inspectCollectLetPatternIdxs` 재귀 walker 로 tuple / struct / binding / or pattern 까지 타고 내려가 destructuring let (`let (a, b) = ...`, `let Point { x, y } = ...`, `let p @ P = ...`) 의 leaf 바인더를 LET 으로 emit. match/destructure 용 pattern binder 만 BIND 로 남음. **남은 갭**: (1) `hintName` top-down 재실행 미구현 — 에이전트 리포트: typeName 문자열 파서 (`List<T>` / tuple / `Map<K,V>` / `fn(..) -> R` 접두어+괄호 스캔) + typedByNode map + AST walker, ~370 LOC scoped. (2) Expression `notes` (generic instantiation, "method call" 마커) 비어있음. (3) Go 브리지 (`selfhost.InspectFromRun`) 는 `bundle.toolchainCheckerFiles` 업데이트 + `generated.go` 재생성 필요 |
