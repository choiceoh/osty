...
> 기준일: 2026-04-26 (post-#929 audit, HEAD = #931). 이전 2026-04-24 스냅샷
> 이후 90+ 커밋이 들어왔다. 코드 실측 기준: `--legacy` check/typecheck escape
> hatch 는 제거됐고, `--native` 는 backwards-compat no-op 이다. `just front`,
> `just verify-selfhost`, CLI astbridge-free 회귀 테스트는 통과 (단
> `verify-selfhost` 는 `internal/ci`/`internal/runner` SnapshotParity 만 — 머지
> 토론체인 MIR 파이프라인은 커버하지 않음). **`osty check toolchain` smoke 는
> 이제 0 errors** (#926 + #927 후); merged native MIR backend gate 는 여전히
> 실패 — 자세한 내용은 아래 "2026-04-26 — 현재 first-walls" 섹션 참조.

## 2026-04-26 — 현재 first-walls (post-#929 실측)

`.bin/osty check toolchain` 은 **0 errors** — 이전 11 type errors 모두 해결:

- ~~`E0702 no field 'typeName'` × 9~~ — **#927 (`fix(selfhost): finish typeName→typeRepr migration in inspect/lint`)** 가 `inspect.osty` / `lint.osty` 9 소비 사이트를 `frontTypeReprToString(.typeRepr)` 경로로 마이그레이션.
- ~~`E0708 missing field 'valueText' / 'targetLabel'` × 2~~ — **#926 (`refactor(llvmgen): rename rendering MirSwitchCase to MirGenSwitchCase`)** 가 `mir_generator.osty` 측 중복 선언을 rename.

`go test ./internal/llvmgen -run TestProbe…` 머지 probe 현재 상태:

| 테스트 | 시간 | First wall |
|---|---|---|
| `TestProbeWholeToolchainMerged` (AST-only) | 3.7s | (PASS) |
| `TestProbeNativeToolchainMerged` (AST, ci.osty 제외) | ~2s | (PASS) |
| `TestProbeNativeToolchainMergedMIR` (full MIR) | ~280s | `LLVM000 unsupported-source: unsupported local type <error> in tyToRepr (local _scrut id=7; written by assign at 216:11 (src=UseRV))` (12 checker diag 발화 후) — 잔존 |
| `TestNativeToolchainMergedMIRErrTypeFloor` | ~270s | **FAIL** — ErrType locals 가 floor=40 초과 (12× 추정, 재측정 필요) — 잔존 |
| `TestNativeToolchainMergedMIRPipelineIsClean` | **236s** | **FAIL** — MIR 가 `ErrUnsupported` 로 거부 후 legacy AST fallback 이 `LLVM011 type-system: struct "CheckFnSig" field "hasReceiver" has a default value` 로 종료. **MIR 거부 원인 (`LLVM000 tyToRepr`) 이 진짜 first wall**; LLVM011 은 legacy fallback 단계에서 보일 뿐. |

**2026-04-26 실측 정정**: `TestNativeToolchainMergedMIRPipelineIsClean` 은
이전에 5분 timeout 으로 보고됐으나 현재 236s 에 fail 함 (post-#929 `-timeout
360s` 환경). `scopeFor` 가 더 이상 timeout 의 직접 원인이 아니다 — 실제 fail
흐름은 *MIR 거부 → legacy fallback → LLVM011* 이다.

`LLVM000 tyToRepr` 가 보이는 이유는 `TestProbeNativeToolchainMergedMIR` 경로
뿐이며 `TestNativeToolchainMergedMIRErrTypeFloor` / 위 PipelineIsClean 도
같은 경로 의존. AST-only 단계 (typeName + MirSwitchCase) 가 클린해진 지금
다음 단계는 **MIR 거부 원인 추적** (LLVM000 tyToRepr 12 diag 의 root cause →
하나씩 fix → MIR pass 통과 → legacy fallback 자체가 안 일어남 → LLVM011
moot). 이전 revision 이 인용한
`TestGoGenerateSelfhostLeavesGeneratedArtifactsClean` red light 는 PR #854
(Osty→Go bootstrap transpiler 폐기) 시점에 테스트 자체가 제거되었으며 더
이상 존재하지 않는다.

**`scopeFor` O(N²) 우선순위 강등 (2026-04-26)**: 이전 revision 이 top wall
로 표시했지만, 5분 timeout 이 재현되지 않으므로 (`-timeout 360s` 에서 236s
완료) algorithmic 한계가 아닌 _상수 큰 hot path_ 일 가능성이 높다. profile
없이 sorted+binary search 로 깎는 것은 premature — 테스트가 거의
4분이라는 사실은 여전히 worth 한 경로지만 critical path 는 LLVM000 쪽이다.
...
- **Phase 0 (immediate, blocks every other gate)**: 위 "현재 first-walls"
  의 11 type errors 해소. (a) `MirSwitchCase` 중복 선언 정리 —
  `mir_generator.osty` 측을 `MirGenSwitchCase` 같은 다른 이름으로 분리하거나
  `mir.osty` 의 정의로 통일. (b) `inspect.osty` / `lint.osty` 의 `typeName`
  레퍼런스를 `typeRepr` 기반으로 마이그레이션 (PR #892 follow-up). (c)
  `selfhostSpanIndex.scopeFor` O(N²) → 정렬+이분 또는 interval tree 로
  교체해 `TestNativeToolchainMergedMIRPipelineIsClean` timeout 해소.
- **Phase 1 (resolver 잔여)**: `pub use` 가시성 (E0553) workspace pub-symbol
  graph. 매트릭스 1c.5 의 `internal/resolve/{resolve,cfg,prelude,scope}.go`
  실제 삭제는 lint / formatter / LSP 의 `*ast.File` identity 소비자 포팅과
  맞물려 진행.
