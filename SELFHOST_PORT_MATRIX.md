...
참조 — 그 중 TOP 3 small-first 후보였던 ~~A13 (`#[pure]`→readnone) LLVM emit~~ 은
2026-05-20 마무리 (위 Annotation semantic validation row 참조). 남은 small-first
후보: E0553 Path A, 숫자 리터럴 다형성 확장, 그리고 같은 mir_json 디코더 누락
패턴으로 dormant 한 A8/A9/A10/A11/A5/A6/A7 annotation emit.
...
- **PR #850** — A8 `#[inline]` annotation native LLVM emit 경로 wire.
...
| Annotation semantic validation | `(hostboundary)` | `toolchain/resolve.osty` (arg validators) + `toolchain/hir_lower.osty:923-950` (extract) + `toolchain/mir.osty::MirFunction` + `toolchain/mir_lower.osty::mirLowerCopyFnAnnotations` (HIR→MIR 전달) | E0739 | **partial (LLVM-emission 남음)** | 2026-04-23 resolve 단계 arg validator 10 종 전부 이식. HIR extract 계층 + HirFnDecl 필드 10 종 landing 완료. **2026-04-23 후속**: MirFunction 이 9 annotation 그룹 필드 (`vectorize[Width/Scalable/Predicate]` / `noVectorize` / `parallel` / `unroll[Count]` / `inlineMode` (MirInlineMode enum) / `hot` / `cold` / `targetFeatures` / `noaliasAll` / `noaliasParams` / `pure`) 을 HirFnDecl 로부터 `mirLowerCopyFnAnnotations` 가 lossless 복사 — Go `ir.FnDecl` shim 이 소비할 수 있는 단일 MIR-level 소스 완성. **2026-05-20 A13 readnone JSON-decode landing**: `toolchain/mir_json.osty::mirJsonFunctionBuildCore` 가 `"pure"` (keyId 4020706) 를 디코드해 `MirFunction.pure` 로 흘림. 그 전까지 `pure` 가 Go→JSON 까지는 살아 있었지만 Osty subprocess 디코더에서 hardcoded `false` 로 드랍되어, `mir_generator.osty::mirEmitFnAttrsFromMir` / `lir_proto.osty::lirFnAttrsFromMir` 의 `readnone` emit 코드가 dead path 였음. `toolchain/mir_json_test.osty` round-trip 회귀 2 종 (pure=true → IR 에 `readnone`, pure=missing → IR 에 `readnone` 부재). **잔여**: 같은 JSON-decode 누락이 `inlineMode` / `hot` / `cold` / `targetFeatures` / `noaliasAll` / `noaliasParams` / `vectorize*` / `parallel` / `unroll*` 에 대해 동일하게 존재 — A8/A9/A10/A11/A5/A6/A7 LLVM emit 활성화하려면 같은 패턴으로 디코더 확장 필요. A11 `noalias` 는 param-level attribute 라 별도 (`lirParamAttrsFromMir` 가 이미 `noaliasAll && pointer` 케이스 emit; 디코더만 추가하면 그대로 활성). 현재는 Go `internal/llvmgen` 쪽이 emit 을 전담 — Osty native LLVMgen 은 여전히 scalar-only |
...
5. **Annotation semantic validation 완성** — `#[vectorize]`/`#[parallel]`/`#[hot]`/`#[pure]` 를 `hir_lower.osty` 에 추가. A5–A13 stanza 기반 스펙이 권위.
