...
> Status note: `examples/gc` is now a prototype/invariant model, not the
> active source of truth for Osty's runtime GC implementation. The live
> implementation path is the LLVM lowering/runtime pair in
> `internal/llvmgen/llvmgen.go` and `internal/backend/runtime/osty_runtime.c`.
> See [`../../RUNTIME_GC.md`](../../RUNTIME_GC.md) for the active path.
...
- `GcHardeningReport`의 readiness score/band는 heap/barrier/performance/contract 상태를
  하나의 운영 지표로 요약해 prototype maturity를 수치로 추적한다.
...
이 섹션은 historical prototype roadmap이다. 실제 구현 우선순위는 이 문서가 아니라
LLVM/runtime 경로와 [`../../RUNTIME_GC.md`](../../RUNTIME_GC.md)가 정한다.
순서는 대략적인 의존성을 따른다.
...
### Phase 43: Forwarding and Async Snapshot Corruption Negatives

- Status: done for the executable prototype.
- Compaction 이후 `forwarding table` entry를 변조했을 때 `validateHeap()`가 즉시 실패하는
  regression을 추가한다.
- Suspended async frame의 lowered root slot은 살아 있지만 snapshot roots에서 빠진 경우를
  validator negative path로 잠근다.
- Humongous free-list와 tenured free-list가 서로 겹치는 corruption case도 regression으로
  고정한다.
- Acceptance criteria: remap metadata와 suspended-frame root snapshot이 어긋나면 executable
  model에서 바로 드러난다.

### Phase 44: Async Lifecycle State Corruption Negatives

- Status: done for the executable prototype.
- Inactive async frame이 `frameObjectId`나 `roots`를 그대로 들고 남는 corruption case를
  `validateHeap()` regression으로 잠근다.
- Suspended async frame의 `safepointId`가 무효한 값으로 깨졌을 때 validator가 즉시 실패하는
  negative path를 추가한다.
- Acceptance criteria: async state machine metadata drift가 collect/resume 이전에 heap validator에서
  바로 드러난다.

### Phase 45: Closure and Suspended-Frame Metadata Negatives

- Status: done for the executable prototype.
- Closure object의 `refs`가 capture descriptor와 어긋나면 `validateHeap()`가 즉시 실패하는
  regression을 추가한다.
- Suspended async frame이 `frameObjectId`를 잃어버린 corruption case도 validator negative path로
  고정한다.
- Acceptance criteria: closure capture metadata와 suspended-frame root object drift가 executable
  model에서 곧바로 드러난다.

### Phase 46: Global Root and Runtime-Frame Metadata Negatives

- Status: done for the executable prototype.
- Global root의 state/object 조합이 깨지거나 key/name metadata가 어긋날 때
  `validateHeap()`가 즉시 실패하는 regression을 추가한다.
- Lowered runtime-frame slot의 owner object kind가 drift했을 때 validator가 바로 실패하는
  negative path를 고정한다.
- Acceptance criteria: global-root table corruption과 runtime-frame ownership drift가 executable
  model에서 collection 이전에 드러난다.

### Phase 47: Stack-Map Kind and Derived-Base Negatives

- Status: done for the executable prototype.
- Safepoint live root kind가 frame layout slot kind와 맞지 않을 때 stack-map build가 즉시
  실패하는 regression을 추가한다.
- Derived root base가 frame layout의 `derivedFrom` metadata와 어긋나는 corruption case도
  negative path로 고정한다.
- Acceptance criteria: stack-map slot taxonomy drift와 derived-root base mismatch가 executable
  model에서 바로 드러난다.

### Phase 48: Async Frame Identity and Root-List Negatives

- Status: done for the executable prototype.
- Async frame table의 key/id mismatch, non-positive id, empty function name corruption을
  `validateHeap()` regression으로 고정한다.
- Suspended async frame root list의 duplicate / non-positive / missing object corruption도
  validator negative path로 추가한다.
- Acceptance criteria: suspended-frame metadata drift가 frame object와 roots를 건드리기 전에
  executable model에서 즉시 드러난다.

### Phase 49: Global Root Dangling-Object Negatives

- Status: done for the executable prototype.
- Global root가 empty module/name metadata를 가지거나 dangling object id를 들고 있는
  corruption case를 `validateHeap()` regression으로 고정한다.
- Invalid negative object id와 missing object reference도 validator가 직접 잡는 범위로
  문서화한다.
- Acceptance criteria: global-root table corruption이 collector reachability와 섞이기 전에
  executable model에서 바로 드러난다.

### Phase 50: Safepoint and Frame-Layout Structural Negatives

- Status: done for the executable prototype.
- Safepoint plan/function metadata, collector capability flags, non-derived root metadata drift를
  `validateSafepointPlan()` regression으로 고정한다.
- Derived frame slot pointer/base corruption과 defer/question/return preserved-root drop도
  stack-map build negative path로 추가한다.
- Acceptance criteria: stack-map contract와 cleanup-root contract drift가 executable model에서
  즉시 드러난다.

### Phase 51: Hardening Readiness Score

- Status: done for the executable prototype.
- `GcHardeningReport`에 heap/barrier/performance/contract readiness sub-score와 overall
  readiness score/band를 추가한다.
- Structural corruption은 `blocked`, performance-only threshold miss는 `experimental`,
  full gate pass는 `hardened`로 바로 읽히게 만든다.
- Acceptance criteria: GC prototype maturity를 binary pass/fail만이 아니라 score로도
  추적할 수 있고, 기존 hardening regression이 readiness degradation까지 함께 검증한다.
## Open Questions
...
- Phase 43은 forwarding metadata, async frame snapshot, humongous/tenured free-list overlap
  corruption까지 negative-path regression으로 고정한다.
- Phase 44는 async lifecycle state corruption까지 validator regression으로 잠가 suspended/running
  frame metadata drift를 더 일찍 드러낸다.
...
6. Phase 52는 이 `runtime/` contract를 실제 bundled native runtime으로 메워서 LLVM 기본 binary path가
   `osty_rt_strings_*`, `osty_rt_list_*`, `osty.gc.*` 심볼을 별도 수작업 없이 바로 링크하고 실행하게 만든다.
7. Phase 53은 bundled runtime을 no-op shim에서 root-tracked managed heap으로 끌어올려
   `List`/split-string/runtime allocation이 tracked object가 되고, debug collect 기준 mark-sweep으로
   liveness와 child tracing을 실제 검증할 수 있게 만든다. 다만 current lowering이 mutable local slot 외의
   immutable/param/safepoint roots를 아직 완전히 내리지 않으므로, 자동 collection은 이후 stack-map 단계까지
   보수적으로 유예한다.
