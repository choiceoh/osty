...
`GcHeap`은 `GcLoweredObjectField` side table을 유지하고, 이 table의 pointer-bearing
...
closure capture rewrite, user object field store lowering을 이 ABI로 낮췄다.
Collection store와 runtime frame store는 Phase 19-20에서 같은 taxonomy를 확장한다.
