...
- Use `just repair-check`, `just ci`, and `just verify-selfhost` for wider validation when touching CLI, toolchain, or generated outputs. For the full source-compiler self-rebuild ratchet (stage2/3 byte parity), use `just verify-self-rebuild`.
- After `just bootstrap`, backend MIR-direct tests can run in CI-strict mode with `OSTY_REQUIRE_REAL_LLVM_EMISSION=1 go test -short ./internal/backend/` (without it, missing `osty-self` skips those tests).
